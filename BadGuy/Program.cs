    ...
    internal static class Program
    {
        ...
        private static void Main()
        {
            if (!_init)
            {
                ...
                herofeeder.AddItem(
                    new MenuItem("Hero.Type", "Feed Type:").SetValue(
                        new StringList(new[] {"attack every creep on midlane", "go to the enemy base"})));
                ...
                courSelection.AddItem(new MenuItem("Courier.Blocking.Enable", "Enable Selected Order").SetValue(false))
                    .DontSave();
                ...
                laugh.AddItem(
                    new MenuItem("laugh.Message", "Print Message on laught").SetValue(false).SetTooltip("only for you"));
                ...
                drawing.AddItem(
                    new MenuItem("Drawing.Enable", "Enable Spam Drawing").SetValue(new KeyBind(0x11, KeyBindType.Press))
                        .SetTooltip("on minimap")
                        .DontSave()).ValueChanged += Program_ValueChanged;


                drawing.AddItem(new MenuItem("Drawing.Speed", "Speed").SetValue(new Slider(4, 1, 10)));
                drawing.AddItem(new MenuItem("Drawing.Cd", "Rate").SetValue(new Slider(40, 1, 1000)));
            ...
            Events.OnLoad += (sender, args) =>
            {
                ...
                _drawingStartPos = new Vector2(10, 809);
                _drawingEndPos = new Vector2(270, 1061);
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            if (Utils.SleepCheck("Hero.Cd") && Menu.Item("Hero.Enable").GetValue<bool>())
            {
                ...
                var angle = (float)Math.Max(
                        Math.Abs(_myHero.RotationRad -
                                 Utils.DegreeToRadian(_myHero.FindAngleBetween(_fountain2.Position))) - 0.20, 0);
                // ReSharper disable once CompareOfFloatsByEqualityOperator
                if (angle > 0)
                {
                    if (Menu.Item("Hero.Type").GetValue<StringList>().SelectedIndex == 0)
                    {
                        _myHero.Attack(_fountain2.Position);
                    }
                    else
                    {
                        _myHero.Move(_fountain2.Position);
                    }
            ...
            {
                var couriers = ObjectManager.GetEntities<Courier>().Where(x => x.IsAlive && x.Team == _myHero.Team);
                ...
                else if (index >= 2)
                {
                    foreach (var courier in couriers)
                    {
                        ...
                        {
                            //Game.PrintMessage("Any items: " + courier.Inventory.Items.Any(x => Equals(x.Owner, _myHero)),MessageType.ChatMessage);
                            //if (courier.Inventory.Items.Any(x => Equals(x.Owner, _myHero)))
