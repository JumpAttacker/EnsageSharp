...
namespace Legion_Annihilation
{
    public static class MenuManager
    {
        ...
        public static void Init()
        {
            ...
            orb.AddItem(new MenuItem("Orbwalking.WhileTargetInStun", "Do Not Use orbwalking if target is stunned")
                .SetValue(true));
            settings.AddItem(
                    new MenuItem("Combo.Enable", "Combo key").SetValue(new KeyBind('D'))).ValueChanged +=
                Core.OnValueChanged;
            ...
            items.AddItem(
                new MenuItem("itemEnableLinken", "Linken breaker:").SetValue(
                    new AbilityToggler(new Dictionary<string, bool>())));
            items.AddItem(
                        new MenuItem("Bkb.Toggle", "BKB toggle").SetValue(new KeyBind('0', KeyBindType.Toggle)))
                    .ValueChanged +=
                Core.BkbToggler;
