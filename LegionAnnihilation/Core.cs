    ...
    public class Core
    {
        ...
        private static async Task Action(CancellationToken cancellationToken)
        {
            ...
            if (notInInvis || !MenuManager.IsInvisEnable)
            {
                ...
                foreach (var item in linkerBreakers) await UseItem(item, target, cancellationToken);

                var itemInCombo =
                    enumerable.Where(
                        x => Helper.IsItemEnable(x) && !target.IsLinkensProtected() && !ComboSleeper.Sleeping(x));
                foreach (var item in itemInCombo) await UseItem(item, target, cancellationToken);
        ...
        private static async Task UseInvis(Hero target, CancellationToken cancellationToken)
        {
            while (true)
            {
                ...
                if (UseHealBeforeInvis && dist <= InvisRange) await UseHeal(cancellationToken);
                ...
                if (dist <= InvisRange)
                {
                    ...
                    {
                        if (!UseHealBeforeInvis) await UseHeal(cancellationToken);
                    }
                }
                else
                {
        ...
        private static async Task UseHeal(CancellationToken cancellationToken)
        {
            ...
            if (heal.CanBeCasted() && Helper.IsAbilityEnable(heal) && Members.MyCurrentMana > heal.ManaCost)
            {
                ...
                var isAoe = Members.MyHero.Spellbook.Spells
                                .FirstOrDefault(x => x.Name == "special_bonus_unique_legion_commander_5")?.Level > 0;
                if (isAoe)
                    heal.UseAbility(Members.MyHero.InFront(150));
                else
                    heal.UseAbility(Members.MyHero);
        ...
        private static async Task UseItem(Ability ability, Unit target, CancellationToken cancellationToken)
        {
            ...
            if (Members.MyCurrentMana < manaCost) return;
        ...
        private static async void GameDispatcherOnOnUpdate(EventArgs args)
        {
            if (!IsEnable)
                return;
        ...
        public static void UpdateItems(EventArgs args)
        {
            ...
            foreach (var item in removeList) Members.Items.Remove(item);
        }
