...
namespace OverlayInformation.Features
{
    public class ShrineHelper
    {
        ...
        private Dictionary<Unit, bool> _underVision;
        private readonly Dictionary<uint, Ability> _abilityDictinart;
        private readonly Dictionary<Unit, ParticleEffect> _effects;
        public ShrineHelper(Config config)
        {
            _underVision = new Dictionary<Unit, bool>();
            _abilityDictinart = new Dictionary<uint, Ability>();
            _effects = new Dictionary<Unit, ParticleEffect>();
            ...
            foreach (var me in Shrines)
            {
                ...
                {
                    _underVision.Add(me, me.IsVisibleToEnemies);
                }
            }
            Entity.OnInt32PropertyChange += (sender, args) =>
            {
                var me = sender as Unit;
                if (me!=null && Shrines.Contains(me))
                {
        ...
        {
            if (Enable)
                foreach (var v in Shrines)
                {
                    var pos = HUDInfo.GetHPbarPosition(v);
                    if (pos.IsZero)
                        continue;
                    var filler = GetFiller(v);
        ...
        private void Callback()
        {
            ...
            foreach (var shrine in Shrines)
            {
                ...
                var inRange = myPos.Distance2D(shrine) <= 700;
        ...
        public IEnumerable<Unit> Shrines { get; set; }
        ...
        private bool CheckForAbility(Unit v)
        {
            Ability s;
            var handle = v.Handle;
        ...
        private Ability GetFiller(Unit v)
        {
            Ability s;
            var handle = v.Handle;
        ...
        private void UnHandleEffect(Unit unit)
        {
        ...
        private void HandleEffect(Unit unit)
        {
            ParticleEffect effect;
            if (!_effects.TryGetValue(unit, out effect))
            {
